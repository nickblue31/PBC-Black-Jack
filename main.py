...
def draw_banker(window, cards):
    ...
    for i in range(len(cards)):
        card = cards[i]
        if not card.open:
            window.blit(back_image, (350 + 60*i, 120))
        else:
            window.blit(images[card.color+card.face], (350 + 60*i, 120))
...
def draw_player(window, cards):
    ...
    for i in range(len(cards)):
        card = cards[i]
        window.blit(images[card.color+card.face], (350 + 60*i, 320))
...
def draw_points(window, cardp, cardb):
    ...
    if cardb.showed:
        ...
        window.blit(bankerpt, (WIDTH/2, HEIGHT/2-225))
    ...
    window.blit(playerpt, (WIDTH/2, HEIGHT/2+130))
...
def main():
    ...
    hitbutton = pokers.Button(200, HEIGHT-80, hitimg)
    hitbutton.draw(window)
    standbutton = pokers.Button(350, HEIGHT-80, standimg)
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                # 發牌(暫時測試用)
                if hitbutton.act(pos, pygame.mouse.get_pressed()):
                    if players_turn:
                        deal(deck, cardp)
                        hitbutton.clicked = False
                elif standbutton.act(pos, pygame.mouse.get_pressed()):
                    if players_turn:
                        players_turn = False
